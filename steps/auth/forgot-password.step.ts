...
export const config: ApiRouteConfig = {
  ...
  method: 'POST',
  emits: ['ForgotPasswordRequested'], // must match event handler subscribes
  description: 'Receives forgot password requests and emits ForgotPasswordRequested event',
  bodySchema: registerInputSchema,
  flows: ['auth'],
};

export const handler: Handlers['ForgotPasswordRequest'] = async (req, { emit, logger }) => {
  const { email } = req.body;

  if (!email) {
    return {
      status: 400,
      body: { error: 'Please provide an email.' },
    };
  }

  logger.info(`[MOTIA] Forgot password request received for ${email}`);

  // ✅ Emit event with correct topic and payload
  await emit({
    topic: 'ForgotPasswordRequested',
    data: { 
  ...
  return {
    status: 200,
    body: { message: 'If an account exists, a password reset link will be sent.' },
  };
};
